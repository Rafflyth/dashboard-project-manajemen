...
function seedSampleData(PDO $pdo)
{
    try {
        $pdo->exec("CREATE TABLE IF NOT EXISTS clients (
            ...
            client_file VARCHAR(255) NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
        $pdo->exec("ALTER TABLE clients ADD COLUMN IF NOT EXISTS client_file VARCHAR(255) NULL AFTER email");
        ...
        $pdo->exec("ALTER TABLE projects ADD COLUMN IF NOT EXISTS project_file VARCHAR(255) NULL AFTER description");
        ...
        $pdo->exec("ALTER TABLE tasks ADD COLUMN IF NOT EXISTS task_file VARCHAR(255) NULL AFTER description");
...
{
    return [
        'pdf', 'doc', 'docx', 'ppt', 'pptx', 'xls', 'xlsx', 'jpg', 'jpeg', 'png', 'webp', 'gif', 'zip', 'rar', 'txt'
    ];
}

function safeUploadFile($uploaded, $targetDir = 'uploads/projects', $maxBytes = 10485760)
{
    if (!is_array($uploaded) || !isset($uploaded['tmp_name']) || !is_uploaded_file($uploaded['tmp_name'])) {
        return null;
    }
    ...
    if (($uploaded['size'] ?? 0) <= 0 || (int)$uploaded['size'] > $maxBytes) {
        return null;
    }

    $originalName = basename((string)($uploaded['name'] ?? ''));
    $extension = strtolower(pathinfo($originalName, PATHINFO_EXTENSION));
    $allowed = getAllowedUploadExtensions();
    ...
    $mimeType = $finfo->file($uploaded['tmp_name']);
    $allowedMimeTypes = [
        'application/pdf',
        'application/msword',
    ...
    $baseDir = __DIR__ . '/' . ltrim($targetDir, '/');
    if (!is_dir($baseDir)) {
    ...
    return ltrim($targetDir, '/') . '/' . $safeName;
}

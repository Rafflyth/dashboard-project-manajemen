...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title && $projectId && $assignedUserId) {
        executeQuery(
            'INSERT INTO tasks (project_id, title, description, assigned_user_id, status, due_date, created_at) VALUES (:project_id, :title, :description, :assigned_user_id, :status, :due_date, NOW())',
            [
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="action-row">
                                            <a class="button-link" href="tasks.php?status=<?php echo urlencode($statusFilter); ?>&detail=<?php echo (int)$task['id']; ?>">Lihat Detail</a>
                                            <?php if (!$isAdmin): ?>
                                                <?php if ($task['status'] !== 'Completed'): ?>
                                                    <a class="button-link" href="tasks.php?complete=<?php echo $task['id']; ?>">Selesaikan</a>
                                                <?php else: ?>
                                                    <span class="badge completed">Selesai</span>
                                                <?php endif; ?>
                                            <?php else: ?>
                                                <a class="button-link danger" href="tasks.php?delete=<?php echo $task['id']; ?>" onclick="return confirm('Hapus tugas ini?');">Hapus</a>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                </tr>
